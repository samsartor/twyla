//! Twyla's native prelude.
//!
//! The all-Rust-builtins seam: `#[func]`/`#[elem]`-defined items injected
//! into typst's *global* scope, callable from a plain `.typ` file with **zero
//! imports and zero `#show`** — the "content dir full of plain typ files"
//! target.
//!
//! How it works:
...
//!
//! Asset URLs are now handled by the real [`crate::asset`] system
//! (`asset.file`/`asset.sass`), which replaced the original `asset-url`
//! fingerprint-the-string placeholder.

use typst::foundations::{Binding, NativeElement};
...
pub fn install(library: &mut Library) {
    ...
    // Overload `document`. Grab the native binding *first* (so the
